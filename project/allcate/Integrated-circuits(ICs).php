...
<body>
    ...
    <?php
    $rows = $conn->query("SELECT * from products where status = 1 AND category = 'integrated-circuits(ICs)'");
    $rows->execute();
    ...
    <section class="sec1">
        <div class="sec_div">
            <p class="path">
                ...
                <a href="integrated-circuits(ICs).php">integrated-circuits(ICs)</a>
            </p>
            <div class="sec_div_1">
                <img id="sensor" src="<?php echo APPURL; ?>/admin-panel/products-admins/image/category/integrated-circuits-ics.png" alt="sensor_logo">
                <div>
                    <h1><b> Integrated-circuits(ICs)</b></h1>
                    <p id="head2">An integrated circuit is a set of electronic circuits in a single chip incorporated together to perform a specific function</p>
                    <p id="head4">Integrated circuit (IC) sometimes called a chip or microchip. It is a wafer of semiconductor material (usually silicon) on which multiple transistors, capacitors and resistors are fabricated.</p>
                </div>
            </div>
            <hr id="hr_line">
        </div>

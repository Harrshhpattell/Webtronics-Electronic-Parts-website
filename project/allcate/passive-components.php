...
<body>
    ...
    <?php
    $rows = $conn->query("SELECT * from products where status = 1 AND category = 'passive-components'");
    $rows->execute();
    ...
    <section class="sec1">
        <div class="sec_div">
            <p class="path">
                ...
                <a href="sensor.php">Passive-components</a>
            </p>
            <div class="sec_div_1">
                <img id="sensor" src="<?php echo APPURL; ?>/admin-panel/products-admins/image/category/passive-components.png" alt="sensor_logo">
                <div>
                    <h1><b> Passive-components</b></h1>
                    <p id="head2">Components that are capable of affecting the flow of an electrical current, but don’t generate, amplify or control it.</p>
                    <p id="head4">The passive component provides attenuation of an electrical signal instead of generation or amplification. It consumes energy but doesn’t produce or control it. Resistors, capacitors, inductors and transformers are the main passive components used in electronics.</p>
                </div>
            </div>
            <hr id="hr_line">
        </div>

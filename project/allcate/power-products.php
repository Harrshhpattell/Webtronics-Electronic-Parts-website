...
<body>
    ...
    <?php
    $rows = $conn->query("SELECT * from products where status = 1 AND category = 'power-products'");
    $rows->execute();
    ...
    <section class="sec1">
        <div class="sec_div">
            <p class="path">
                ...
                <a href="sensor.php">power-products</a>
            </p>
            <div class="sec_div_1">
                <img id="sensor" src="<?php echo APPURL; ?>/admin-panel/products-admins/image/category/power-products.png" alt="sensor_logo">
                <div>
                    <h1><b>Power Products</b></h1>
                    <p id="head2">Devices used for providing electric power to an electrical load, transforming electric energy, protecting electrical loads</p>
                    <p id="head4">Power products are the wide range of devices that provide electric power to electrical load, transform parameters of an electric energy, protect the load from dangerous changes in power supply (overvoltage, overcurrent, power supply interrupt).</p>
                </div>
            </div>
            <hr id="hr_line">
        </div>
